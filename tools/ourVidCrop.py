...
root_path = '/home/zyf/code/RetargetVid-main'
...
sys.path.insert(0, '/home/zyf/code/Saliency-Ranking-main')
...
salient_cfg = setup(args)
salinet_model = build_model(salient_cfg)
salient_model_root_dir = "/home/zyf/code/Saliency-Ranking-main/tools/Rank_Saliency/Models/RVSOR(52)/"
salient_model_names = 'model_0019999.pth'
model_dir = os.path.join(salient_model_root_dir, salient_model_names)
DetectionCheckpointer(salinet_model, save_dir=salient_cfg.OUTPUT_DIR).resume_or_load(
    model_dir, resume=args.resume)
import inferece_from_video
salinet_model.eval()
...
def read_and_segment_video(video_path, crop_params, verbose=False):
	...
	frames = np.zeros((batch_size, 
						SAL_H, SAL_W, 3), 
						dtype=np.uint8)
	frames_salient = np.zeros((batch_size, 
						SAL_H, SAL_W, 3), 
						dtype=np.uint8)
	...
	while fvs.more():
		...
		if frame is None:
			...
			frames_salient[bsi,:,:,:] = cv2.resize(frame, 
									(SAL_W, SAL_H),
		...
		if cur_batch_len==batch_size or bail_out:
			...
			for i in range(cur_batch_len):
				...
				if ((si+i==true_inds[-1]+skip) if len(true_inds)>0 else True) or \
				   ...
				   (si+i==frame_count-1):
					...
					frames_salient[process_ind] = frames_salient[i]
			...
			# 											frames[:process_ind,:,:,:], [], '')
			vid_data['smaps'][:,:,si:ei] = inferece_from_video.do_inference(salient_cfg, 
																   salinet_model,frames_salient[:process_ind,:,:,:])
			# for pp in range(si,ei):
			# 	if np.sum(vid_data['smaps'][:,:,pp]) > 0:
			# 		pass
			# 	else:
			# 		print(pp)
			# exit()
			#print(vid_data['smaps'][:,:,si:ei])
			# print(vid_data['smaps'].shape)
			# for img_id in range(si,ei):
			# 	cv2.imwrite('./test/'+str(img_id)+'.png', vid_data['smaps'][:,:,img_id])
			# exit()
...
def sc_find_center_of_mass(sal_map, km=True, factor=2.0, bias=1.0, verbose=False):
	...
	# cv2.imwrite('/home/zyf/code/Saliency-Ranking-main/tools/retarget/'+str(0)+'out.jpg', sal_map)
	# exit()

	h1, w1 = sal_map.shape
	contours, cnt = cv2.findContours(sal_map, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
	...
	for i in range(len(contours)):
		try:
			...
			grey=sal_map[center_y][center_x]
		except:
			continue
		center_dit.append(grey)
		X_center.append(center_x)
		Y_center.append(center_y)
		# cv2.drawContours(image, contours, i, int(grey), -1)  # 绘制轮廓，填充
		# m_image=cv2.circle(image, (center_x, center_y), 7, 128, -1)  # 绘制中心点
	x,y=0,0
	for i in range(len(X_center)):
		x+=X_center[i]*center_dit[i]
		y+=Y_center[i]*center_dit[i]
	if sum(center_dit)==0:
		pass
	else:
		x=x/sum(center_dit)
	...
	# x,y,c=0,0,0
	# for i in range(len(X_center)):
	# 	if max(center_dit)==center_dit[i]:
	# 		x+=X_center[i]
	# 		y+=Y_center[i]
	# 		c+=1
	# if c==0:
...
if __name__ == '__main__':
	...
	aspect_ratios_to_test = ['4:5','3:4','2:3','1:2','16:9']
	temp_path = None
	...
	# setup tests
	tests = {}
	tests['our_RVSOR'] = crop_params_test.copy()
	print(' Tests::')
	...
	# del vid_paths[101:601]
	# del vid_paths[201:len(vid_paths)]
	# del vid_paths[100]
	vids_in_dir = '/home/zyf/code/Saliency-Ranking-main/dataset/RVSOD/RVSOD/test/videos/'
	vid_paths = []
	if vid_overide is not None:
		vid_paths = [os.path.join(vids_in_dir, vid_overide)]
	else:
	...
	for test_name in tests.keys():
		for iorp, orp in enumerate(aspect_ratios_to_test):  # '4:5', '1:1'
			...
			#for i, vid_path in enumerate([vid_paths[0]]):
			#for i, vid_path in enumerate(vid_paths[:20]):
				...
				try:
							...
							for user in range(6):
								...
								for iframe, bb in enumerate(vid_data['bbs']):
									...
									if print_eval_frames:
										...
										# width_out = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))    #获取视频的宽度
										# height_out = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))   #获取视频的高度
										# fps_out = cap.get(cv2.CAP_PROP_FPS)    #获取视频的帧率
										# fourcc_out = int(cap.get(cv2.CAP_PROP_FOURCC))    #视频的编码
										# video_out = cv2.VideoWriter("test./output.avi", fourcc_out, fps_out, (width_out, height_out))
										# for frame_ in frame:
										# 	video_out.write(frame_)
										# video_out.release()
										#cv2.imwrite('./test/1.jpg', frame)
					...
					print('Wrong video catch:',vid_path)
					continue
