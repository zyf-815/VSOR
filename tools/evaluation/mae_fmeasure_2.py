...
def make_map(gt_rank, rank_score, gt_masks, segmaps):
    ...
    for i in range(len(segmaps)):
        # for j in range(len(segmaps[i])):
        #     for k in range(len(segmaps[i][j])):
        #         if segmaps[i][j][k] > 0.5:
        #             if rank_map[j][k]==0:
        #                 rank_map[j][k]=rank_index[i]
        #             else:
        #                 rank_map[j][k]=(rank_index[i]+rank_map[j][k])/2.0
        rank_map[segmaps[i] > 0.5] = rank_index[i]
        # rank_map[segmaps[i] >0.5] = 1
    for i in range(len(gt_masks)):
        # for j in range(len(gt_masks[i])):
        #     for k in range(len(gt_masks[i][j])):
        #         if gt_masks[i][j][k] !=0:
        #             if gt_map[j][k]==0:
        #                 gt_map[j][k]=gt_index[i]
        #             else:
        #                 gt_map[j][k]=(gt_index[i]+gt_map[j][k])/2.0
        gt_map[gt_masks[i] != 0] = gt_index[i]
        # gt_map[gt_masks[i] !=0] = 1
