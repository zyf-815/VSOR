...
def generate(dataset):
    ...
    for i in range(len(dataset)):
        try:
            k = dataset[i][1]["file_name"].split('/')[-3]
            if k in dit:
                dit[k].append(i)
            else:
                print(k)
...
def inference2(cfg, model,draw=False):
    dataset = davis_val(cfg, False)
    video_dit=generate(dataset)
    print(len(dataset))
    Core = []
    Map = []
    Mae = []
    ...
    with inference_context(model), torch.no_grad():
        for key in video_dit:
            res = []
            res_mae=[]
            no_figure=0
            for i in video_dit[key]:
                try:
                    inputs = [dataset[i]]
                    ...
                    print('\r{}/{}'.format(len(res), len(video_dit[key])), end="", flush=True)
            ...
            r_corre,num_ = rank_evalu(res, 0.5)
            r_map=float(mf_map(res))
            r_f = float(mf_evalu(res_mae)['mae'])
            Core.append(r_corre)
            Map.append(r_map)
            Mae.append(r_f)
            Num.append(num_)
            Name.append(key)
            print(key + '  ' + 'r_corre: ' + str(r_corre) + '   r_map: ' + str(r_map) + '   mae: ' + str(r_f))
        out = {'video':Name , 'corre':Core , 'mae':Mae ,'map':Map , 'num':Num}
        out=pd.DataFrame(out)
        out.to_csv('D_Train.csv')
    return 0, {"mae": 0, "f_measure": 0},0

def generate2(dataset):
    # select_0609 select_0290 select_0572 select_0236 select_0345 select_0674 select_0543 select_0408 select_0328 select_0057 select_0521 select_0247
    dit = []
    for i in range(len(dataset)):
        try:
            k = dataset[i][1]["file_name"].split('/')[-3]
            if k =='select_0609' or k =='select_0290' or k =='select_0572' or k =='select_0236' or k =='select_0345' or k =='select_0674' or k =='select_0543' or k =='select_0408' or k =='select_0328' or k =='select_0057' or k =='select_0521' or k =='select_0247':
                dit.append(i)
    ...
    with inference_context(model), torch.no_grad():
        ...
        # for i in range(10):
            try:
                # if i not in need:
                #     continue
                inputs = [dataset[i+k] for k in range(batch)]
                outputs = model(inputs)
                # continue
                ...
                # saliency_rank = []
                # for temp in pred_boxes:
                #     saliency_rank.append([((temp[2]-temp[0])*(temp[3]-temp[1]))])
                ...
                if draw:
                    ...
                    if os.path.exists('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_box/' + file_name) :
                        pass
                    else:
                        os.makedirs('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_box/' + file_name)
                    cv2.imwrite('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_box/' + file_name + '/' +img_name.split('/')[-1], imgs_c)
                    ...
                    if os.path.exists('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_mask/' + file_name) :
                        pass
                    else:
                        os.makedirs('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_mask/' + file_name)

                    cv2.imwrite('/home/zyf/code/Saliency-Ranking-main/tools/draw/8-8/DAVSOD/draw_mask/' + file_name+ '/'+img_name.split('/')[-1], imgs_d)
        ...
        # exit()
        print('wrong figure=',no_figure)
        r_corre,num = rank_evalu(res, 0.5)
        r_map=mf_map(res)
